...
void RateCalulationWatBal() {
    ...
    if (WatBal->st.SurfaceStorage <= 0.1) 
    {
        ...
        if (Site->InfRainDependent) WatBal->rt.Infiltration = 
               (1. - Site->NotInfiltrating * Afgen(Site->NotInfTB, &Rain[Lon][Lat])) * 
               Rain[Lon][Lat] + WatBal->rt.Irrigation + WatBal->st.SurfaceStorage / Step;
        else
            RINPRE = (1. - Site->NotInfiltrating) * Rain[Lon][Lat] + 
    ...
    else 
    {
        ...
        /* rate root zone */
        Available = WatBal->st.SurfaceStorage + (Rain[Lon][Lat] * 
